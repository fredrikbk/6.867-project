...
The HMM classifier is trained using the values $Y_{1}...Y_{t}$ and then used to predict value $Y_{t+1}$.
...
We then performed standard message passing along the chain from $Z_{t}$ to $Y_{t+1}$ to evaluate the marginal $p(Y_{t+1})$, which we used to predict $Y_{t+1}$.
Figure~\ref{fig:hmm-predict} illustrated this model.

\begin{figure}
\center
\includegraphics[width=6cm]{figs/hmm-predict.pdf}
\caption{Predicting the movement of the Dow Jones the next day ($Y_{t+1}$) using a Hidden Markov Model.}
